...
train       = EasyDict(func='train.train_progressive_gan', resume_run_id=-1)  # Options for main training func.
...
#train = EasyDict(func='util_scripts.generate_fake_images', run_id=9, num_pngs=250); num_gpus = 1; desc = 'fake-images-' + str(train.run_id)
...
train = EasyDict(func='util_scripts.evaluate_metrics', run_id=10, log='metric-fid-10k.txt', metrics=['fid'], num_images=10000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
...
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=10, log='metric-is-50k.txt', metrics=['is'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
